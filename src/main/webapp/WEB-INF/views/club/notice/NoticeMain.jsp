				...
				<table class="table table-boardead table-hover" style="text-align: center;">
					...
					<tbody>
						<c:forEach items="${list}" var="dto">
			               <tr align="center">
			               	  <td></td>
			               	  <td>${dto.main_notice_no}</td>
			                  <td>
			                  	<a href="/club/notice/read?main_notice_no=${dto.main_notice_no}">${dto.main_notice_subject}</a>
			                  	<c:if test="${dto.replycount > 0}">[${dto.replycount}]</c:if>
				                <fmt:parseNumber var="date" value="${(now.time - dto.main_notice_regdate.time)/(1000*60*60*24*60)}" integerOnly="true"/>
				                <c:if test="${date > 0 }">
										<img src="/resources/images/ico-new.gif"/>	
				                </c:if>
			                  </td>
			                  <td>${dto.main_notice_writer}</td>
			                  <td>
			                 	 <fmt:formatDate value="${dto.main_notice_regdate}" pattern="yyyy/MM/dd hh:mm:ss"/>
			                  </td>
			                  <td>${dto.main_notice_count}</td>
			               </tr>
	              		</c:forEach>
					</tbody>
				</table>
					...
					<div class="row">
							...
							<div class="col-md-4">
								<ul id="pagingul" class="pagination">
									...
									<c:if test="${pageMaker.prev}">
										<li><a 
											href="/club/notice/main${pageMaker.makeSearch(pageMaker.startPage - 1) }">&laquo;</a></li>
									</c:if>
									...
									<c:forEach begin="${pageMaker.startPage}" 
										...
										<li
											...
											<a href="/club/notice/main${pageMaker.makeSearch(idx)}">${idx}</a>
										</li>
									</c:forEach>
									
									<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
										<li><a 
											href="/club/notice/main${pageMaker.makeSearch(pageMaker.endPage + 1) }">&raquo;</a></li>
									</c:if>
								</ul>
							</div>
						</div>
						...
						<script>
							$('#searchBtn').on("click", function(event) {
								...
								self.location = "/club/notice/main${pageMaker.makeQuery(1)}&searchType="+$("select option:selected").val() + "&keyword=" + $('#keywordInput').val();
							});
						</script>
...
<script>
	...
	function fnWrite(){
		...
		if(user_key!=null && user_key!=""){
			location.replace("/club/notice/register");
		}
		else{
			alert("로그인 하세요.");
		}
