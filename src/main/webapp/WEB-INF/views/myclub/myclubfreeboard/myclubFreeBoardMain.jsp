				...
				<table class="table table-boardead table-hover" style="text-align: center;">
					...
					<tbody>
						<c:forEach items="${list}" var="myclubDTO">
			               <tr align="center">
			               	  <td></td>
			               	  <td>${myclubDTO.myclub_free_no}</td>
			                  <td><a href="/myclub/free/read?myclub_free_no=${myclubDTO.myclub_free_no}">
			                  		${myclubDTO.myclub_free_subject}</a>
			                  		<c:if test="${myclubDTO.replycount > 0}"> [${myclubDTO.replycount}]</c:if>
			                  		
			                  		<fmt:parseNumber var="date" value="${(now.time - myclubDTO.myclub_free_regdate.time)/(1000*60*60*24*60)}" integerOnly="true"/>
			                  		<c:if test="${date > 0}">
			                  			<img src="/resources/images/ico-new.gif"/> 
			                  		</c:if> 
			                  </td>
			                  <td>${myclubDTO.myclub_free_writer}</td>
			                   <td><fmt:formatDate value="${myclubDTO.myclub_free_regdate}" pattern="yyyy/MM/dd hh:mm:ss"/></td>
			                  <td>${myclubDTO.myclub_free_count}</td>
			               </tr>
	              		</c:forEach>
					</tbody>
				</table>
							...
							<div class="col-md-4">
								<ul id="pagingul" class="pagination">
									...
									<c:if test="${pageMaker.prev}">
										<li><a 
											href="/myclub/free/main${pageMaker.makeSearch(pageMaker.startPage - 1) }">&laquo;</a></li>
									</c:if>
									...
									<c:forEach begin="${pageMaker.startPage}" 
										...
										<li
											...
											<a href="/myclub/free/main${pageMaker.makeSearch(idx)}">${idx}</a>
										</li>
									</c:forEach>
									
									<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
										<li><a 
											href="/myclub/free/main${pageMaker.makeSearch(pageMaker.endPage + 1) }">&raquo;</a></li>
									</c:if>
								</ul>
							</div>
						</div>
						...
						var msg = "${msg}";
						   ...
						   function fnWrite(){
							...
							$('#searchBtn').on("click", function(event) {
								...
								self.location = "/myclub/free/main${pageMaker.makeQuery(1)}&searchType="+$("select option:selected").val() + "&keyword=" + $('#keywordInput').val();
							});
						</script>
