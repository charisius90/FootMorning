					...
					<c:forEach items="${list}" var="myclubDTO" varStatus="status">
		                  <c:if test="${not empty myclubDTO.myclub_album_main_thumnail}">
			                  <div class="col-xs-6 col-md-3" >
									<a href="/myclub/album/read?myclub_album_no=${myclubDTO.myclub_album_no}" class="thumbnail" style="height: 180px">${myclubDTO.myclub_album_no}
								   	<img src="${myclubDTO.myclub_album_main_thumnail}" style="max-width: 200px; max-height: 140px" /></a>
							 	제목 : ${myclubDTO.myclub_album_subject}	
							 		<c:if test="${myclubDTO.replycount > 0}"> [${myclubDTO.replycount}]</c:if> 
							 		
							 		<fmt:parseNumber var="date" value="${(now.time - myclubDTO.myclub_album_regdate.time)/(1000*60*60*24*60)}" integerOnly="true"/>
			                  		<c:if test="${date > 0}">
			                  			<img src="/resources/images/ico-new.gif"/>
			                  		</c:if> 
							 	<br/>
		                		작성자 : ${myclubDTO.myclub_album_writer}<br/>
		                		날짜 : <fmt:formatDate value="${myclubDTO.myclub_album_regdate}" pattern="yyyy/MM/dd hh:mm:ss"/><br/><br/>
							  </div></c:if>
              		</c:forEach>
              	</div>
				<br/>
						...
						<div class="col-md-4">
							<ul id="pagingul" class="pagination">
									...
									<c:if test="${pageMaker.prev}">
										<li><a 
											href="/myclub/album/main${pageMaker.makeSearch(pageMaker.startPage - 1) }">&laquo;</a></li>
									</c:if>
									...
									<c:forEach begin="${pageMaker.startPage}" 
										...
										<li
											...
											<a href="/myclub/album/main${pageMaker.makeSearch(idx)}">${idx}</a>
										</li>
									</c:forEach>
									
									<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
										<li><a 
											href="/myclub/album/main${pageMaker.makeSearch(pageMaker.endPage + 1) }">&raquo;</a></li>
									</c:if>
								</ul>
						</div>
						...
						<script>
						   ...
						   function fnWrite(){
						      ...
						      if(user_key!=null && user_key!=""){
						         location.replace("/myclub/album/register");
						      }
						      else{
						         alert("로그인 하세요.");
						      }
							...
							$('#searchBtn').on("click", function(event) {
								...
								self.location = "/myclub/album/main${pageMaker.makeQuery(1)}&searchType="+$("select option:selected").val() + "&keyword=" + $('#keywordInput').val();
							});
						</script>
