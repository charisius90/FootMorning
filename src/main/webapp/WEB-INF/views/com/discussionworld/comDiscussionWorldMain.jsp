					...
					<tbody>
						<c:forEach items="${list}" var="dto">
			               <tr align="center">
			                  ...
			                  <td>
			                  	<a href="/com/world/read?com_discussion_world_no=${dto.com_discussion_world_no}">${dto.com_discussion_world_subject}</a>
			                  	<c:if test="${dto.replycount > 0}">[${dto.replycount}]</c:if>
				                <fmt:parseNumber var="date" value="${(now.time - dto.com_discussion_world_regdate.time)/(1000*60*60*24*60)}" integerOnly="true"/>
				                <c:if test="${date > 0 }">
					...
					<div class="row">
							...
							<div class="col-md-4">
								<ul id="pagingul" class="pagination">
									...
									<c:if test="${pageMaker.prev}">
										<li><a 
											href="/com/world/main${pageMaker.makeSearch(pageMaker.startPage - 1) }">&laquo;</a></li>
									</c:if>
									...
									<c:forEach begin="${pageMaker.startPage}" 
										...
										<li
											...
											<a href="/com/world/main${pageMaker.makeSearch(idx)}">${idx}</a>
										</li>
									</c:forEach>
									
									<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
										<li><a 
											href="/com/world/main${pageMaker.makeSearch(pageMaker.endPage + 1) }">&raquo;</a></li>
									</c:if>
								</ul>
							</div>
						</div>
						...
						<script>
							$('#searchBtn').on("click", function(event) {
								...
								self.location = "/com/world/main${pageMaker.makeQuery(1)}&searchType="+$("select option:selected").val() + "&keyword=" + $('#keywordInput').val();
							});
						</script>
...
<script>
	...
	function fnWrite(){
		...
		if(user_key!=null && user_key!=""){
			location.replace("/com/world/register");
		}
		else{
			alert("로그인 하세요.");
		}
