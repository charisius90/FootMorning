					...
					<div class="row">
							...
							<div class="col-md-4">
								<ul id="pagingul" class="pagination">
									...
									<c:if test="${pageMaker.prev}">
										<li><a 
											href="/com/qna/main${pageMaker.makeSearch(pageMaker.startPage - 1) }">&laquo;</a></li>
									</c:if>
									...
									<c:forEach begin="${pageMaker.startPage}" 
										...
										<li
											...
											<a href="/com/qna/main${pageMaker.makeSearch(idx)}">${idx}</a>
										</li>
									</c:forEach>
									
									<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
										<li><a 
											href="/com/qna/main${pageMaker.makeSearch(pageMaker.endPage + 1) }">&raquo;</a></li>
									</c:if>
								</ul>
							</div>
						</div>
						...
						<script>
							$('#searchBtn').on("click", function(event) {
								...
								self.location = "/com/qna/main${pageMaker.makeQuery(1)}&searchType="+$("select option:selected").val() + "&keyword=" + $('#keywordInput').val();
							});
						</script>
...
<script>
	...
	function fnWrite(){
		...
		if(user_key!=null && user_key!=""){
			location.replace("/com/qna/register");
		}
		else{
			alert("로그인 하세요.");
		}
